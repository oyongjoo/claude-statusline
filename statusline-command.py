...
ctx_size = get_val(data, 'context_window.context_window_size', '0')
...
if cwd and os.path.isdir(cwd):
    session_id = re.sub(r'[^A-Za-z0-9_-]', '', get_val(data, 'session_id', ''))
...
if effort and effort != 'none':
    mtxt += ' ' + c(DIM, effort)
if get_val(data, 'thinking.enabled', False):
    mtxt += ' ' + c(DIM, 'think')
seg.append(mtxt)
